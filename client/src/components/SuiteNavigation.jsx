...
export default function SuiteNavigation({ 
  ...
  const isCollapsed = isDockVisible !== undefined ? !isDockVisible : internalCollapsed;
  const setCollapsed = (val) => {
    if (setIsDockVisible) {
      setIsDockVisible(!val);
    }
    setInternalCollapsed(val);
    localStorage.setItem('wagent_suite_dock_collapsed', String(val));
  };
  ...
  return (
    <>
      {/* ========================================================================= */}
      {/* DESKTOP FLOATING SUITE DOCK (Modo Compacto / Modo Expandido)             */}
      {/* ========================================================================= */}
      {isCollapsed ? (
        /* Píldora Mínima Discreta (Totalmente no-intrusiva, libera toda la pantalla) */
        <div className="hidden lg:flex fixed bottom-3 left-1/2 -translate-x-1/2 z-40 items-center gap-2 px-3 py-1.5 bg-[#111b21]/90 backdrop-blur-xl border border-slate-700/80 hover:border-red-500/50 rounded-full shadow-2xl shadow-black/70 transition-all duration-300 select-none group">
          <button
            onClick={() => setCollapsed(false)}
            className="flex items-center gap-2 text-xs font-semibold text-slate-300 hover:text-white"
            title="Mostrar barra inferior de navegación (WAgent Suite)"
          >
            <div className="w-5 h-5 rounded-full bg-gradient-to-tr from-red-600 to-amber-500 flex items-center justify-center text-white text-[10px] shadow-sm group-hover:scale-110 transition">
              <Flame size={12} />
            </div>
            <span className="text-[11px] font-bold text-white tracking-wide">
              {currentApp?.label || 'WAgent Suite'}
            </span>
            <span className="flex items-center gap-1 text-[10px] text-slate-400 bg-slate-800/80 px-2 py-0.5 rounded-full group-hover:bg-red-600 group-hover:text-white transition">
              <Eye size={11} />
        ...
        /* Dock Completo con Botón para Minimizar / Ocultar */
        <div className="hidden lg:flex fixed bottom-4 left-1/2 -translate-x-1/2 z-40 items-center gap-1.5 p-2 bg-[#111b21]/92 backdrop-blur-xl border border-slate-700/80 rounded-3xl shadow-2xl shadow-black/60 transition-all duration-300 hover:border-red-500/50 animate-in fade-in slide-in-from-bottom-3">
          ...
          <div className="h-6 w-[1px] bg-slate-700/80 mx-0.5" />

          {/* Botón Ocultar / Minimizar Barra Inferior */}
          <button
            onClick={() => setCollapsed(true)}
            className="p-2 rounded-2xl text-slate-400 hover:text-red-400 hover:bg-slate-800/60 transition"
            title="Ocultar barra inferior (Modo pantalla limpia y despejada)"
          >
            <EyeOff size={16} />
          </button>
